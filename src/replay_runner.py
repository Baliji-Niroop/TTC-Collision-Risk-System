...
def _save_replay_charts(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(11, 4.8))
    ax.plot(df.index, df["predicted_confidence"], label="Predicted confidence", color="#d62728")
    ax.plot(df.index, df["confidence"], label="Packet confidence", color="#ff7f0e", alpha=0.7)
    ...
    fig, ax = plt.subplots(figsize=(11, 4.8))
    ax.step(df.index, df["predicted_risk_class"], where="post", label="Predicted risk", color="#b71c1c")
    ax.step(df.index, df["risk_class"], where="post", label="Packet risk", color="#1f77b4", alpha=0.8)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Replay a telemetry CSV session through the TTC pipeline.")
    parser.add_argument("--input", type=str, default=str(DATA_PATH), help="Session CSV or packet log to replay")
    parser.add_argument("--output-dir", type=str, default=str(LOG_DIR / "replay_outputs"), help="Directory for replay artifacts")
    parser.add_argument("--interval-ms", type=float, default=0.0, help="Delay between packets during replay")
    ...
    for _, row in session_df.iterrows():
        ...
        predicted_risk_class, predicted_confidence = predict_risk_with_confidence(parsed, model)
        ...
        action = recommend_action(predicted_risk_class, packet_for_alerts["ttc_basic"], predicted_confidence)
        collision_probability = calculate_collision_probability(packet_for_alerts["ttc_basic"], predicted_confidence)
    ...
    summary = {
        ...
        "critical_events": int((replay_df["predicted_risk_class"] == 2).sum()) if len(replay_df) else 0,
        "warning_events": int((replay_df["predicted_risk_class"] == 1).sum()) if len(replay_df) else 0,
        "safe_events": int((replay_df["predicted_risk_class"] == 0).sum()) if len(replay_df) else 0,
        "mean_ttc": float(replay_df["ttc_basic"].mean()) if len(replay_df) else 0.0,
        "mean_confidence": float(replay_df["predicted_confidence"].mean()) if len(replay_df) else 0.0,
