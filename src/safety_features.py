...
class VelocitySanityFilter:
    ...
    def validate_velocity(
        self, speed_kmh: float, timestamp: float
...
class MLConfidenceFusion:
    ...
    def predict_risk_fused(
        self, ttc_basic: float, ml_confidence: float, physics_confidence: float = 0.95
    ) -> Tuple[int, float, str]:
        """
        ...
        final_confidence = (
            final_weight_physics * physics_confidence + final_weight_ml * ml_confidence
        )
    ...
    def set_weights(self, physics_weight: float, ml_weight: float):
        ...
        logger.info(
            f"Fusion weights set: physics={self.physics_weight:.2f}, ml={self.ml_weight:.2f}"
        )
...
class DatasetAutoLabeller:
    ...
    def label_row(
        self, row: Dict[str, Any], prediction_info: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """
        ...
        if ttc > 3.0:
            label = 0  # SAFE
            confidence = min(
                0.95, 1.0 - (ttc - 3.0) * 0.01
            )  # Higher TTC = higher confidence
        elif ttc > 1.5:
            label = 1  # WARNING
