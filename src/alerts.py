    ...
    def trigger_alert(self, risk_class: int, row: dict) -> bool:
        ...
        try:
            if risk_class == 2:  # CRITICAL
                ...
                if (
                    self.critical_count_since_alert
                    >= ALERT_CONFIG["critical_alert_every_n_events"]
                ):
                    should_alert = True
                ...
                if (
                    self.warning_count_since_alert
                    >= ALERT_CONFIG["warning_alert_every_n_events"]
                ):
                    should_alert = True
            ...
            if ttc < ALERT_CONFIG["min_ttc_alert_threshold"]:
                should_alert = True
                alert_msg = (
                    f"[IMMINENT] COLLISION - TTC: {ttc:.2f}s - BRAKE IMMEDIATELY"
                )
